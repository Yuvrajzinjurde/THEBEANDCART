...
export async function GET(req: Request) {
    try {
        ...
        const cart = await Cart.findOne({ userId }).populate('items.productId', 'name images sellingPrice');
        if (!cart) {
...
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json({ message: 'Product added to cart successfully', cart: populatedCart }, { status: 200 });
  } catch (error) {
