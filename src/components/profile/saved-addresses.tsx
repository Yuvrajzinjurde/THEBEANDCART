        ...
        <Form {...form}>
            <form onSubmit={form.handleSubmit(handleFormSubmit)} className="space-y-4">
                ...
                {addressType === 'Other' && (
                 ...
                 <FormField control={form.control} name="isDefault" render={({ field }) => (<FormItem className="flex items-center gap-2"><FormControl><Checkbox checked={field.checked} onCheckedChange={field.onChange} /></FormControl><Label>Set as default address</Label></FormItem>)} />
                 <DialogFooter>
...
export function SavedAddresses({ user, onUserUpdate }: SavedAddressesProps) {
    ...
    const [selectedAddress, setSelectedAddress] = useState<AddressFormValues | undefined>(undefined);
    const [addressToDelete, setAddressToDelete] = useState<string | null>(null);
    
    const openForm = (address?: IAddress) => {
        setSelectedAddress(address ? JSON.parse(JSON.stringify(address)) : undefined);
    ...
    const saveAddress = async (formData: AddressFormValues) => {
        ...
        if (selectedAddress?._id) { // Editing existing address
            const index = updatedAddresses.findIndex(a => a._id.toString() === selectedAddress._id);
            if (index !== -1) updatedAddresses[index] = formData as IAddress;
        } else { // Adding new address
            updatedAddresses.push(formData as IAddress);
        }
    ...
    return (
        <Card className="shadow-lg">
            <CardHeader className="flex flex-row items-start justify-between">
                ...
                <Button onClick={() => openForm()}><PlusCircle className="mr-2 h-4 w-4" /> Add New</Button>
            </CardHeader>
            <CardContent>
                        ...
                        user.addresses.map(addr => (
                            <div key={addr._id as string} className={cn("p-4 border rounded-lg flex justify-between items-start", addr.isDefault && "border-primary border-2")}>
                                ...
                                <div className="flex flex-col gap-2">
                                     <Button variant="outline" size="sm" onClick={() => openForm(addr)}><Edit className="h-4 w-4" /></Button>
                ...
                <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                    <DialogContent>
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                Your address information is safe with us.
                            </DialogDescription>
                        </DialogHeader>
                        <AddressForm 
